...
from fleet.tasks.teardown_test_vcluster import main
...
@mock.patch("fleet.tasks.teardown_test_vcluster.subprocess.run")
def test_teardown_success(mock_run):
...
@mock.patch("fleet.tasks.teardown_test_vcluster.subprocess.run")
def test_namespace_delete_tolerant(mock_run):
    mock_run.side_effect = [_ok(), _fail(), _ok()]
    with mock.patch(
        "sys.argv",
        ["prog", "--cluster-name", "test-vc", "--namespace", "test-ns"],
    ):
